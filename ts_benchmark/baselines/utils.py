...
class DatasetForTransformer:
    ...
    def __read_data__(self):
        ...
        # `self.dataset.index` enthält die Datums-Timestamps.
        # Wir müssen sie nicht aus einer Spalte extrahieren.
        df_stamp = self.dataset.index.values.reshape(1, -1)
    ...
    def __getitem__(self, index):
        ...
        # Slicing direkt aus dem Original-DataFrame
        seq_x_df = self.dataset.iloc[s_begin:s_end]
        seq_y_df = self.dataset.iloc[r_begin:r_end]
        
        # Slicing der vorbereiteten Zeit-Features
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]

        # --- ANFANG DER KORREKTUR ---
        # Konvertiere nur die reinen Werte (ohne den Index) in einen Tensor.
        seq_x_values = seq_x_df.values
        seq_y_values = seq_y_df.values
        
        seq_x = torch.tensor(seq_x_values, dtype=torch.float32)
        seq_y = torch.tensor(seq_y_values, dtype=torch.float32)
        # --- ENDE DER KORREKTUR ---
