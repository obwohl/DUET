...
class DUET(ModelBase):
    ...
    def forecast_fit(
        ...
        # Instantiate SummaryWriter for TensorBoard logging
        log_dir = f'runs/duet_experiment_{int(time.time())}'
        ...
        scheduler = None
        if config.lradj == "plateau" and train_ratio_in_tv != 1:
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, "min", patience=config.patience // 2, factor=0.5
            )
        ...
        try:
            for epoch in range(config.num_epochs):
                ...
                for _, (input, target, input_mark, target_mark) in enumerate(
                    train_data_loader
                ):
                    optimizer.zero_grad()
                    input, target, input_mark, target_mark = (
                    ...
                    criterion_losses.append(loss.item())
                    importance_losses.append(loss_importance.item())

                avg_train_loss = np.average(train_losses)
                avg_criterion_loss = np.average(criterion_losses)
                avg_importance_loss = np.average(importance_losses)
                writer.add_scalar("Loss/train_total", avg_train_loss, epoch)
                writer.add_scalar("Loss/train_criterion", avg_criterion_loss, epoch)
                writer.add_scalar("Loss/train_importance", avg_importance_loss, epoch)
